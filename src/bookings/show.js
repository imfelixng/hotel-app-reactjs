...
class Bookingshow extends Component {
    ...
    onSubmit = (e) => {
        ...
        BookingModel.updateBooking(this.props.match.params.id, {
    ...
    getBooking = () => {
        ...
        BookingModel.getBooking(this.props.match.params.id).then(res => {
            this.setState({
                ...
                kids: res.data.kids,
                checkin: Moment.unix(res.data['checkin']).format("DD-MM-YYYY"),
                checkout: Moment.unix(res.data['checkout']).format("DD-MM-YYYY"),
                status: res.data.status,
                selectedRoom: res.data.rooms.map(room => room.id),
                selectedRoomObj: res.data.rooms,
                loading: false
            })
            this.data['checkin'] = res.data['checkin'];
            this.data['checkout'] = res.data['checkout'];
        });
    }
            ...
            <div>
                {
                    ...
                    (
                        ...
                        {
                            this.state.selectedRoomObj && this.state.selectedRoomObj.length > 0 && 
                            <div className="row">

                                {
                                    this.state.selectedRoomObj.map((roomItem, index) => (
                                            <div key = {index} className= "card col-sm-4 mb-3">
                                                <div className="card-body">
                                                    <h5 className="card-title">Phòng số {roomItem.number}</h5>
                                                    <div className="card-text">
                                                        <p>Loại phòng: {this.typeRooms[roomItem.type]}</p>
                                                        <p>Số người tối thiểu: {roomItem.min}</p>
                                                        <p>Số người tối đa: {roomItem.max}</p>
                                                    </div>
                                                </div>
                                            </div>
